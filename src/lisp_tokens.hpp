...
namespace Munchar {
  namespace Tokens {
    ...
    constexpr auto comment        = CHR(';') ^ *(!eol ^ _) ^ ~eol;
    constexpr auto spaces              = CLS(" \t\r");

    constexpr auto ellipsis            = STR("...");
    constexpr auto arrow         = STR("->");
    ...
    constexpr auto peculiar_identifier = CLS("+-.") | (arrow ^ *subsequent);
    constexpr auto subsequent = initial | digit | special_subsequent;
    constexpr auto constituent = letter; // and unicode stuff too
